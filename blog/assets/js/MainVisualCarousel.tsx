...
export default function MainVisualCarousel() {
	...
	return (
		<div className="lg:h-[70vh] h-[40vh] w-screen relative">
			<button
				className="absolute top-1/2 lg:left-36 left-4 bg-gray-200 lg:size-12 size-8 rounded-full z-10 cursor-pointer"
				onClick={() => handleClick(-1)}
			>
				←
			</button>
			<button
				className="absolute top-1/2 lg:right-36 right-4 bg-gray-200 lg:size-12 size-8 rounded-full z-10 cursor-pointer"
				onClick={() => handleClick(1)}
			>
				→
			</button>
			...
			<div className="w-screen h-full overflow-hidden">
				...
				>
					{carouselableImagePaths.map(path => {
						return (
							<div className="w-screen h-full shrink-0 flex justify-center items-center">
			...
			<div className="flex justify-center gap-4 absolute lg:bottom-4 bottom-8 inset-x-0">
